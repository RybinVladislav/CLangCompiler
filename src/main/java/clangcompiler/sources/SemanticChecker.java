...
public class SemanticChecker {
    ...
    private static void convert(Tree node, DataType dataType)
        {
            AstNode convert = new AstNode(new CommonToken(CGrammarLexer.CALL, "CONVERT"));
        ...
        public static void checkBlock(Tree node, Context context) throws Exception
        {
            ...
            switch (node.getType())
            {
                ...
                case CGrammarLexer.VAR: {
                    ...
                    for (int i = 0; i < node.getChild(0).getChildCount(); i++)
                    {
                        ...
                        if (temp.getToken().getType() == CGrammarLexer.ASSIGN)
                        {
                            ...
                            AstNode var = new AstNode(new CommonToken(CGrammarLexer.VAR, "VAR"));
                ...
                case CGrammarLexer.IDENT:
                {
                    ...
                    if (ident.getIdentType() == IdentType.Function)
                    {
                        ...
                        AstNode call = new AstNode(new CommonToken(CGrammarLexer.CALL));
                        call.addChild(node);
                ...
                case CGrammarLexer.TRUE:
                case CGrammarLexer.FALSE:
                {
                ...
                case CGrammarLexer.LT:
                {
                    ...
                    if (leftDataType == DataType.Double)
                    {
                        if (rightDataType == DataType.Int)
                            convert((AstNode)node.getChild(1), DataType.Double);
                        node.setDataType(compareOperation ? DataType.Bool : DataType.Double);
                        return node.getDataType();
                    }
                    if (rightDataType == DataType.Double)
                    {
                        if (leftDataType == DataType.Int)
                            convert((Tree)node.getChild(0), DataType.Double);
                        node.setDataType(compareOperation ? DataType.Bool : DataType.Double);
                        return node.getDataType();
                    }
                    node.setDataType(compareOperation ? DataType.Bool : DataType.Int);
                    return node.getDataType();
                }
